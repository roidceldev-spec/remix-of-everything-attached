...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.app_accounts WHERE id = NEW.client_id AND role = 'client') THEN
    RAISE EXCEPTION 'Chat client must be a Client account';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.app_accounts WHERE id = NEW.coach_id AND role = 'coach') THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.chat_threads WHERE id = NEW.thread_id
...
DECLARE thread_id uuid; coach_id uuid;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.app_accounts WHERE id = p_client_id AND role = 'client') THEN
    RAISE EXCEPTION 'Client account was not found';
  END IF;
  SELECT id INTO coach_id FROM public.app_accounts WHERE role = 'coach' LIMIT 1;
  IF coach_id IS NULL THEN RAISE EXCEPTION 'Coach account was not found'; END IF;
  SELECT id INTO thread_id FROM public.chat_threads WHERE client_id = p_client_id;
  IF thread_id IS NULL THEN
    INSERT INTO public.chat_threads (client_id, coach_id) VALUES (p_client_id, coach_id)
    ON CONFLICT (client_id) DO NOTHING RETURNING id INTO thread_id;
    IF thread_id IS NULL THEN
      SELECT id INTO thread_id FROM public.chat_threads WHERE client_id = p_client_id;
    END IF;
  END IF;
...
CREATE OR REPLACE FUNCTION public.send_chat_message(
  p_message_id uuid, p_sender_account_id uuid, p_client_id uuid, p_body text
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.chat_threads WHERE id = target_thread_id
...
CREATE OR REPLACE FUNCTION public.mark_chat_read(p_account_id uuid, p_client_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE thread_id uuid;
BEGIN
  thread_id := public.get_or_create_chat_thread(p_client_id);
  IF NOT EXISTS (
    SELECT 1 FROM public.chat_threads WHERE id = thread_id
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT thread.client_id, count(message.id)::bigint AS unread_messages
  FROM public.chat_threads thread
  LEFT JOIN public.chat_reads read_state
    ON read_state.thread_id = thread.id AND read_state.account_id = p_account_id
